...
class ClaudeService {
    ...
    initializeClient() {
        if (!this.client) {
            ...
            if (!apiKey) {
                throw new errorHandler_1.AppError('ANTHROPIC_API_KEY is not configured. Please set up the API key in your environment variables.', 500);
            }
            logger_1.logger.info(`Initializing Claude with API key: ${apiKey.substring(0, 4)}...${apiKey.substring(apiKey.length - 4)}`);
    ...
    async generateTeachingResponse(userMessage, context) {
        try {
            ...
            if (error.status === 401) {
                throw new errorHandler_1.AppError('Claude API authentication failed. Please check API key.', 401);
            }
            else if (error.status === 429) {
                throw new errorHandler_1.AppError('Claude API rate limit exceeded. Please try again later.', 429);
            }
            else if (error.status === 400) {
                throw new errorHandler_1.AppError('Invalid request to Claude API. Please check your input.', 400);
            }
            else {
                throw new errorHandler_1.AppError(`Claude API error: ${error.message || 'Unknown error'}`, 500);
            }
        }
    }
    async generateAssessment(topic, level, questionCount = 5, questionTypes = ['multiple_choice', 'true_false', 'short_answer']) {
        try {
            const client = this.initializeClient();
...
Format the response as a JSON array of questions.`;
            ...
            try {
                ...
                logger_1.logger.error('Failed to parse assessment JSON:', parseError);
                throw new errorHandler_1.AppError('Failed to generate valid assessment format', 500);
            }
        }
        catch (error) {
            logger_1.logger.error('Assessment generation error:', error);
            throw new errorHandler_1.AppError('Failed to generate assessment', 500);
        }
    }
...
Format as JSON.`;
    ...
    async generateLessonContent(topic, duration, targetAudience, includeActivities = true) {
        try {
...
Format as structured JSON.`;
            ...
            logger_1.logger.error('Lesson content generation error:', error);
            throw new errorHandler_1.AppError('Failed to generate lesson content', 500);
        }
    }
    buildTeachingAssistantPrompt(context) {
