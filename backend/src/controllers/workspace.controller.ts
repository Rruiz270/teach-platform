...
export class WorkspaceController {
  ...
  // Create complete lesson with AI
  createLesson = async (req: AuthRequest, res: Response) => {
    try {
      const userId = req.user!.id;
      const { topic, grade, duration, objectives } = req.body;

      // Step 1: Generate lesson outline with Claude
      const outline = await this.aiOrchestrator.executeTask(
        userId,
        'text',
        {
      ...
      // Step 2: Generate lesson content sections
      const content = await this.aiOrchestrator.executeTask(
        userId,
        'text',
        {
      ...
      const assessment = await this.aiOrchestrator.executeTask(
        userId,
        'text',
        {
      ...
      // Step 5: Save everything to database with auto-tracking
      ...
      // Auto-track as teacher project/achievement
  ...
  createVideo = async (req: AuthRequest, res: Response) => {
    try {
      const userId = req.user!.id;
      const { script, avatarId, background, duration } = req.body;

      // Queue video generation (heavy task)
  ...
  getUsageAnalytics = async (req: AuthRequest, res: Response) => {
    try {
      const userId = req.user!.id;
      ...
      // Calculate time saved (estimate: 1 AI task = 15 minutes saved)
  ...
  // Smart content suggestions based on curriculum
  getContentSuggestions = async (req: AuthRequest, res: Response) => {
    try {
      const userId = req.user!.id;
      const { moduleId, lessonType } = req.query;

      // Get user's teaching context
      ...
      const suggestions = await this.aiOrchestrator.executeTask(
        userId,
        'text',
        {
  ...
  private getProviderName(toolName: string): string {
    const providerMap = {
